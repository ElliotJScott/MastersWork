...
progenitorInit = np.array((13.5, 0, 0, 0, 18.16, 0)) #initial values of the problem
...
def ProgenitorPropogator(u,t, mm, msr): #u[0], u[1], u[2] are x,y,z | u[3],u[4],u[5] are vx vy vz
    xNew = u[3]
    yNew = u[4]
    zNew = u[5]
    gdPl = GradPlummer(np.array((u[0], u[1], u[2])), np.array((0,0,0)), mm, msr)
    vxNew = -gdPl[0]
    vyNew = -gdPl[1]
    vzNew = -gdPl[2]
...
def CalcQualityFactor(data):
    ...
    progenitorDataT = igr.odeint(ProgenitorPropogator, progenitorInitT, tValsT, args=((Ms, a)))
...
currIndex = 0
lastT = 0
progenitorData = igr.odeint(ProgenitorPropogator, progenitorInit, tVals, args=((mainMass, mainScaleRadius)))
...
old_tau = np.inf
filename = "mcdynmpo.h5"
backend = emcee.backends.HDFBackend(filename)
backend.reset(nwalkers, ndim)
p0 = (np.random.rand(nwalkers, ndim) * np.array((0.2,0.2, 1000, 1))) + \
    np.array((np.max(gahO), np.min(vDataO), 7000, 7))
...
def ThreadMethodY(dat, m,a,r,v):
    global qfs
    print(str(m) + " " + str(a) + " " + str(dat[2]) + " " + str(dat[3]))
    qfa = CalcQualityFactor(dat)
    qfs[m,a,r,v] = qfa
    expFile = open("griddatadd.txt", "a")
    expFile.write(str(dat[2]) + "|" + str(dat[3]) + "|" + str(qfa) + "\n")
    expFile.close()
        ...
        for a in range(len(aValsM)):
            for r in range(len(rValsM)):
                for v in range(len(vValsM)):
                    ThreadMethodY((rValsM[r],vValsM[v],mValsM[m],aValsM[a]),m,a,r,v)
                #xtt = threading.Thread(target=ThreadMethodY, args=((rValsM[r],vValsM[v],mValsM[m],aValsM[a]),m,a,r,v))
                #xtt.start()
                #qfs[m,a,r,v] = CalcQualityFactor()
