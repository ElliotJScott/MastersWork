    ...
    for pf in range(len(phases)):
        ...
        for q in range(len(pd)):
            if np.abs(pd[q] - p) <= rg:
                ...
                rvc.append(rv[q])
        try:
            tdat, covd = curve_fit(GaussianToFit, pdc, rvc, p0 = np.array((p, -1, 15)))
            add = True
            if np.sign(tdat[1]) == 1:
                continue
...
def CalcAngleMM(x,y,z,phase,progAngle):
    ...
    for m in range(10):
        ...
        for q in range(len(phase)):
            if np.abs(phase[q]) < 50 * (m+1):
                linphase.append(phase[q])
                linangle.append(adjangles[q])
        df, hmf = curve_fit(strln, linphase, linangle, p0=np.array((-0.1,0)))
...
def CalcQualityFactor(data):
    ...
    fullangdatT, ign1, ign2 = CalcAngleMM(xDataT[:,numStepsT-1], yDataT[:,numStepsT-1], zDataT[:,numStepsT-1], phaseDataT[:,numStepsT-1], CalcAngle(progenitorDataT[numStepsT-1,0], progenitorDataT[numStepsT-1,1], progenitorDataT[numStepsT-1,2]))
    numangbinsT = 400
...
fullangdat, ld, ggl = CalcAngleMM(xOther, yOther, zOther, phaseOther, CalcAngle(progXDataO[0], progXDataO[1], progXDataO[2]))
...
def animate(i):
    ...
    if i < numSteps or True:
        ...
        ad,ofdf, sdngiogn = CalcAngleMM(xd, yd, zd, pd, CalcAngle(progenitorData[i,0], progenitorData[i,1], progenitorData[i,2]))
