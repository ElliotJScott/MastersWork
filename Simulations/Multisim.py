...
def CalcMeanArray(ad, rd, sp, nb):
    amd = np.zeros(nb+1)
    amc = np.zeros(nb+1)
    igrs = np.round((ad-ad.min())  / sp)
    ada =  np.round(ad  / sp) * sp
    ama = np.zeros(nb+1)
    for q in range(len(ada)):
        index = int(igrs[q] + 0.01)
        amd[index] += rd[q]
        amc[index] +=1
    for m in range(len(amd)):
        ama[m] = ad.min() + (m * sp)
        if amc[m] == 0:
            continue
        amd[m] /= amc[m]
...
def CalcGaussianMidPoint(pd, mpd, rv, sp, nb):
    ...
    spacing = 20
    for q in range(len(meanArray)):
        if q >= 1 and q < len(meanArray) - 1:
            if meanArray[q] == 0 and meanArray[q-1] != 0 and meanArray[q+1] != 0:
                meanArray[q] = 0.5 * (meanArray[q-1] + meanArray[q+1])
    for q in range(len(meanArray)):
        if q >= spacing and q <= len(meanArray) - (spacing + 1):
            ...
            for p in range(-spacing, spacing+1):
                if meanArray[p+q] > meanArray[q]:
                    maxim = False
            if maxim == True and meanArray[q] > 0 and np.abs((q + indT.min()) * sp) < 3.5:
                maxindices.append(q)
...
def CalcGaussianSubMidPoint(pd, mpd, rv, sp, nb):
    ...
    for q in range(len(meanArray)):
        if q >= spacing and q <= len(meanArray) - (spacing + 1):
            minim = True
            for p in range(-spacing, spacing+1):
                if meanArray[p+q] < meanArray[q] and meanArray[p+q] != 0:
                    minim = False
            if minim == True and meanArray[q] > 0 and np.abs((q + indT.min()) * sp) < 3.5:
                minindices.append(q)
    ...
    for pf in range(len(phases)):
        ...
        for q in range(len(pd)):
            if np.abs(pd[q] - p) <= rg:
                ...
                rvc.append(rv[q])
        try:
            tdat, covd = curve_fit(GaussianSubToFit, pdc, rvc, p0 = np.array((p, 6, 6)))
            add = True
            if np.sign(tdat[1]) == -1:
                continue
            for q in range(len(angsUsed)):
...
def CalcQualityFactor(data):
    ...
    angdataltT, angmeanangleT, angmeandatT = CalcMeanArray(fullangdatT, rDataT[:,numStepsT-1], fullangdatsepT, numangbins)

    gmpsT = CalcGaussianMidPoint(fullangdatT, angdataltT, rDataT[:,numStepsT-1], fullangdatsepT, numangbins)
    gpamsT = CalcGaussianParams(gmpsT, fullangdatT, rDataT[:,numStepsT-1], fullangdatsepT * 15)
    gsubmpsT = CalcGaussianSubMidPoint(fullangdatT, angdataltT, rDataT[:,numStepsT-1], fullangdatsepT, numangbins)
    gsubpamsT = CalcGaussianSubParams(gsubmpsT, fullangdatT, rDataT[:,numStepsT-1], fullangdatsepT * 15)
    mma = 3.5
    qf = 1
    gam = []
    gah = []
    ...
    amacT = []
    amdcT = []
    for q in range(len(angmeanangleT)):
        if np.abs(angmeanangleT[q]) < 2.5:
            amacT.append(angmeanangleT[q])
            amdcT.append(angmeandatT[q])
    qf *= np.abs((freqT - freqO) / freqO) * np.abs((ampT - ampO) / ampO)
    qfex = np.product(np.abs((veldaT - veldaO) / veldaO))
    qf *= np.power(qfex, 0.5)
    #sldfT, oddT = curve_fit(strln, amacT, amdcT, p0=np.array((-0.3,6)))
    gradT, mpT = CalcMMGradMPs(gam, gah, gbm, gbh)
    qfgmp = (mpT - mpO)/mpO
    qf *= np.power(qfgmp, 2)
...
def CalcMMGradMPs(gam, gah, gbm, gbh):
    gaml, gamh, gbml, gbmh = -1000, 1000, -1000, 1000
    gamlr, gamhr, gbmlr, gbmhr = 1000,1000,1000,1000
    ...
    """
    grada = 0
    for q in range(len(gam) - 1):
        grada += (gah[q+1] - gah[q]) / (gam[q+1] - gam[q])
    grada /= (len(gam) - 1) 
    gradb = 0
    for q in range(len(gbm) - 1):
        gradb += (gbh[q+1] - gbh[q]) / (gbm[q+1] - gbm[q])
    gradb /= (len(gbm) - 1) 
    #grada = (gamhr - gamlr) / (gamh - gaml)
    #gradb = (gbmhr - gbmlr) / (gbmh - gbml)
    grad = (grada + gradb) / 2
    """
    mpl = (gamlr + gbmlr) / 2
    if gamlr > 100 and gbmlr < 100:
        mpl = gbmlr
    elif gbmlr > 100 and gamlr < 100:
        mpl = gamlr
    mph = (gamhr + gbmhr) / 2
    if gamhr > 100 and gbmhr < 100:
        mph = gbmhr
    elif gbmhr > 100 and gamhr < 100:
        mph = gamhr
    mp = (mpl + mph) / 2
    if mpl > 100 and mph < 100:
        mp = mph
    elif mph > 100 and mpl < 100:
        mp = mpl
    return 0, mp
...
def CalcGradOffs(gam, gah, gbm, gbh):
    return 0
...
angdatalt, angmeanangle, angmeandat = CalcMeanArray(fullangdat,rOther, fullangdatsepO, numangbins)
amacO = []
amdcO = []
for q in range(len(angmeanangle)):
    if np.abs(angmeanangle[q]) < mmaO:
        amacO.append(angmeanangle[q])
        amdcO.append(angmeandat[q])
#sldf, odd = curve_fit(strln, amacO, amdcO, p0=np.array((-0.1,6)))
...
gradO, mpO = CalcMMGradMPs(gamO, gahO, gbmO, gbhO)
...
p0 = (np.random.rand(nwalkers, ndim) * np.array((0.2,0.2, 4000, 4))) + \
    np.array((np.max(gahO), np.min(vDataO), 8000, 8))
...
scngngn = ax3[0].scatter(phaseOther, angdat, s=5, marker="o")
scngngn = ax3[1].scatter(phaseOther, fullangdat, s=5, marker="o")
#scap = ax3[0].scatter(np.zeros(numParticles), np.zeros(numParticles), s=5, marker="o")
"""
scngngn = ax3.scatter(phaseOther, ggl, s=5, marker="o")
gjgjg = ax3.plot(xdl, ydl)
gjgjg = ax2.plot(xdla, ydla)
"""
scpo = ax2.scatter(fullangdat, rOther, s=5, marker="o", color="b", alpha=0.9) 
scpo2 = axph.scatter(fullangdat, rOther, s=5, marker="o", color="b", alpha=0.9) 
#scpo = ax2.scatter(angmeanangle, angmeandat, s=5, marker="o", color="r") 

scc = ax[0].scatter(0,0,s=10,marker="x")
alpha = (CalcAngle(progenitorData[numSteps-1,0], progenitorData[numSteps-1,1], 0) - CalcAngle(progXDataO[0], progXDataO[1], progXDataO[2])) + 0.6
alpha = 0
sco = ax[1].scatter((xOther * np.cos(alpha)) +  (yOther * np.sin(alpha)), -(xOther * np.sin(alpha)) +  (yOther * np.cos(alpha)), s=5, marker="o", alpha = 1, color="b")
scxyxy2 = axxy.scatter((xOther * np.cos(alpha)) +  (yOther * np.sin(alpha)), -(xOther * np.sin(alpha)) +  (yOther * np.cos(alpha)), s=5, marker="o", alpha = 1, color="b")
scxyxy = axxy.scatter(initOffsetX,initOffsetY, s=5, marker="o", alpha = 1, cmap = 'RdPu')

scp2 = axxy.scatter(progenitorData[0,0], progenitorData[0,1], s=10, marker="o")
...
scrpd = ax2.scatter(np.zeros(numParticles), np.zeros(numParticles), s=5, marker="o", alpha = 0.5)
scrpd2 = axph.scatter(np.zeros(numParticles), np.zeros(numParticles), s=5, marker="o", alpha = 0.5)
qfaco = CalcQualityFactor(np.array((progenitorInit[0], progenitorInit[4], mainMass, mainScaleRadius)))

text1a = ax[0].text(0.01, 0.99, 'Lagrange Point Stripping',
        verticalalignment='top', horizontalalignment='left',
        transform=ax[0].transAxes,
        color='red', fontsize=15)
text1b = ax[1].text(0.01, 0.99, 'N-Body',
        verticalalignment='top', horizontalalignment='left',
...
axxy.set_xlim(xmin,xmax)
axxy.set_ylim(ymin, ymax)
...
ax2.set_ylim([0, 20])
axph.set_xlim([-5.5,5.5])
axph.set_ylim([0, 20])
ax3[0].set_xlim([-1000,1000])
ax3[0].set_ylim([-10, 10])
ax3[1].set_xlim([-1000,1000])
ax3[1].set_ylim([-10, 10])
ax3[0].set_xlabel("Phase")
ax3[0].set_ylabel("Angle")
ax3[1].set_xlabel("Phase")
ax3[1].set_ylabel("Angle")
ax2.set_xlabel("Angle")
ax2.set_ylabel("r")
axph.set_xlabel("Angle")
axph.set_ylabel("r")
ax4.set_xlim([0, 400])
ax4.set_ylim([0,100])
...
cs = ax4.contourf(Xm, Ym, qfs[mminind,aminind,:,:].T, 20, cmap=cm.PuBu_r)
cs = ax4.contourf(Xm, Ym, qfs[:,:,rminind,vminind].T, 20, cmap=cm.PuBu_r)
...
def animate(i):
    ...
    if i < numSteps or True:
        ...
        scxyxy.set_offsets(np.c_[xd, yd])
        ...
        scxyxy.set_color(scuff2)
        #scEL.set_color(scuff2)
        scxy.set_color(["r"] * numParticles)
        #scEL.set_color(["r"] * numParticles)
        ad = CalcAngleMM(xd, yd, zd, pd, CalcAngle(progXDataO[0], progXDataO[1], progXDataO[2]))
        ...
        scp2.set_offsets(np.c_[progenitorData[i,0], progenitorData[i,1]])
        scrpd.set_offsets(np.c_[ad, rd])
        scrpd.set_color(scuff2)
        scrpd.set_color(["r"] * numParticles)
        scrpd2.set_offsets(np.c_[ad, rd])
        scrpd2.set_color(scuff2)
